    ...
    def _push_aux(self,q,previous,value):
        ...
        if node._left is not None and node._left._value>node._value:
            ...
            if node._left._left is None and node._left._right is None:
                self._last=node._left

        elif node._right is not None and node._right._value>node._value:
            node=self._swap_right(node,previous)
            if node._right._right is None and node._right._left is None:
                self._last=node._right
        return node,inserted
        
    def pop(self):
    	assert self._root is not None, "Can't pop an empty heap"
    	value=self._root._value
    	self._count-=1
    	self._root=self.pop_aux(self._root,None,self._last)
    	return value
    
    def _pop_aux(self,node,previous,last):
    	pass
